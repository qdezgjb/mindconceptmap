...
## 🔴🔴🔴 最重要的规则：首层节点必须与焦点问题直接相关
- **第一层（L1）必须是焦点问题的核心主题本身**
  - 例如：焦点问题是"辛亥革命的背景"，则L1节点就是"辛亥革命背景"
  - 例如：焦点问题是"光合作用的过程"，则L1节点就是"光合作用过程"
  - **错误示例**：焦点问题是"辛亥革命的背景"，L1却是"政治角度"或"辛亥革命"（这些应该放在L2）
- **第二层（L2）必须是焦点问题的直接分类维度**
  - 这些维度必须能够直接回答焦点问题
  - 例如："辛亥革命的背景"的L2应该是：政治背景、经济背景、思想背景、社会背景等

## 层级结构说明（可根据内容自然确定层数，不限制在4层）：
- **第一层（L1）**：焦点问题的核心主题（通常1个节点，必须与焦点问题完全对应）
- **第二层（L2）**：主要分类或维度（必须是L1的直接分解，能够回答焦点问题）
- **第三层（L3）**：具体分类或子维度
- **第四层（L4）**：具体细节或实例
...
# JSON格式示例（以"辛亥革命的背景"为例，13个节点）
# 注意：L1节点必须是焦点问题的核心主题本身，L2节点必须是直接分类维度
{{
  "nodes": [
    {{"id": "1", "label": "辛亥革命背景", "type": "main", "description": "第一层核心节点-必须与焦点问题直接对应", "importance": 10, "layer": 1}},
    {{"id": "2", "label": "政治背景", "type": "core", "description": "第二层-焦点问题的直接分类维度", "importance": 8, "layer": 2}},
    {{"id": "3", "label": "经济背景", "type": "core", "description": "第二层-焦点问题的直接分类维度", "importance": 8, "layer": 2}},
    {{"id": "4", "label": "思想背景", "type": "core", "description": "第二层-焦点问题的直接分类维度", "importance": 8, "layer": 2}},
    {{"id": "5", "label": "社会背景", "type": "core", "description": "第二层-焦点问题的直接分类维度", "importance": 8, "layer": 2}},
    {{"id": "6", "label": "清政府腐败", "type": "detail", "description": "第三层扩展概念", "importance": 6, "layer": 3}},
    {{"id": "7", "label": "列强侵略", "type": "detail", "description": "第三层扩展概念", "importance": 6, "layer": 3}},
    {{"id": "8", "label": "民族资本主义发展", "type": "detail", "description": "第三层扩展概念", "importance": 6, "layer": 3}},
    {{"id": "9", "label": "西方思想传播", "type": "detail", "description": "第三层扩展概念", "importance": 6, "layer": 3}},
    {{"id": "10", "label": "戊戌变法失败", "type": "detail", "description": "第四层细化概念", "importance": 4, "layer": 4}},
    {{"id": "11", "label": "民族危机加深", "type": "detail", "description": "第四层细化概念", "importance": 4, "layer": 4}},
    {{"id": "12", "label": "资产阶级壮大", "type": "detail", "description": "第四层细化概念", "importance": 4, "layer": 4}},
    {{"id": "13", "label": "革命思想传播", "type": "detail", "description": "第四层细化概念", "importance": 4, "layer": 4}}
  ],
  "links": [
    {{"source": "1", "target": "2", "label": "包括", "type": "relation", "strength": 8}},
    {{"source": "1", "target": "3", "label": "包括", "type": "relation", "strength": 8}},
    {{"source": "1", "target": "4", "label": "包括", "type": "relation", "strength": 8}},
    {{"source": "1", "target": "5", "label": "包括", "type": "relation", "strength": 8}},
    {{"source": "2", "target": "6", "label": "表现为", "type": "relation", "strength": 6}},
    {{"source": "2", "target": "7", "label": "表现为", "type": "relation", "strength": 6}},
    {{"source": "3", "target": "8", "label": "表现为", "type": "relation", "strength": 6}},
    {{"source": "4", "target": "9", "label": "表现为", "type": "relation", "strength": 6}},
    {{"source": "6", "target": "10", "label": "导致", "type": "relation", "strength": 4}},
    {{"source": "7", "target": "11", "label": "导致", "type": "relation", "strength": 4}},
    {{"source": "8", "target": "12", "label": "促进", "type": "relation", "strength": 4}},
    {{"source": "9", "target": "13", "label": "推动", "type": "relation", "strength": 4}}
  ],
  "metadata": {{"keyword": "{keyword}", "summary": "概念图摘要", "domain": "领域"}}
}}

# 重要说明
- 🔴🔴🔴 **第一层（L1）节点必须是焦点问题"{keyword}"的核心主题本身**，不能是其他概念
- 🔴🔴🔴 **第二层（L2）节点必须是焦点问题的直接分类维度**，能够直接回答焦点问题
- 节点数量根据内容自然确定，不设限制
- 确保层级结构清晰，从高层到低层
...
## 最终检查清单：
✓ 🔴 L1节点是否与焦点问题"{keyword}"直接对应？（最重要！）
✓ 🔴 L2节点是否是焦点问题的直接分类维度？（例如：背景的各个方面、过程的各个阶段）
✓ 每个节点都有layer属性
...
## 🔴🔴🔴 最重要的规则：首层节点必须与文本核心主题直接相关
- **第一层（L1）必须是文本的核心主题本身**
- **第二层（L2）必须是核心主题的直接分类维度**

## 层级结构说明（可根据内容自然确定层数，不限制在4层）：
- 第一层：核心主题（必须与文本的中心问题直接对应）
- 第二层：主要分类或维度（必须是第一层的直接分解）
- 第三层：具体分类或子维度
- 第四层：具体细节或实例
